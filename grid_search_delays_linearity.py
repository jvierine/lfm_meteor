...
DAN_CENTER_US = 338.176
WEN_CENTER_US = 330.906
DELTA_US = 100.0
STEP_US = 20.0
OUTPUT_H5 = os.path.join("results", "delay_grid_search_linearity.h5")
OUTPUT_PNG = os.path.join("results", "delay_grid_search_linearity.png")
...
def robust_line(points):
    ...
    for _ in range(2):
        ...
        distances = np.linalg.norm(np.cross(points - center, direction), axis=1)
    ...
    direction = vh[0] / np.linalg.norm(vh[0])
    distances = np.linalg.norm(np.cross(points - center, direction), axis=1)
    return center, direction, distances
...
def main():
    ...
    for i, dan_delay0_us in enumerate(dan_grid):
        for j, wen_delay0_us in enumerate(wen_grid):
            score_km, mean_alt_km, median_alt_km = score_grid(trajectories, float(dan_delay0_us), float(wen_delay0_us))
            ...
            print(
                ...
                f"score={score_km:9.3f} km mean_alt={mean_alt_km:7.3f} km median_alt={median_alt_km:7.3f} km"
            )
    ...
    cb.set_label("Sum of point-to-line distances (km)")
    ...
    ax.set_title("Tri-static Trajectory Linearity Grid Search")
    fig.tight_layout()
    ...
    print(f"Best linearity score: {best['score_km']:.3f} km")
